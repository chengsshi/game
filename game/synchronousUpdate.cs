...
namespace Game
{
    class SynchronousUpdate
    {
        ...
        public static void Process(Function type)
        {
                ...
                for (int ite = 0; ite < Parameter.iteration; ++ite) //while current_iteration_number <= maxIteration 
                {
                    ...
                    for (int idx = 0; idx < Parameter.population; ++idx)
                    {
                        ...
                        else
                        {
                            ...
                            for (int dim = 0; dim < Parameter.dimension; ++dim)
                            {
                                groupSynchronous[idx, dim] = population[idx, dim];
                            }
                        }
                    }
                    ...
                    for (int idx = 0; idx < Parameter.population; ++idx)
                    {
                        for (int dim = 0; dim < Parameter.dimension; ++dim)
                        {
                            population[idx, dim] = groupSynchronous[idx, dim];
                        }
                    }
                    #endregion // selection                                            
            ...
            logSentence = type.ToString() + " sy bsoos "+ diffTime + " seconds for " + Parameter.run.ToString() + " runs";
            log.Add(logSentence);
